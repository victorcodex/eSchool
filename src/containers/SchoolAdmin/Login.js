...
class SchoolAdminLogin extends Component {
    constructor(props) {
        super(props);
        this.state = {
            email: '',
    ...
    processLogin = () => { // process login
        ...
        fetch( GeneralApis.API_SCHOOL_ADMIN_BASEURL + GeneralVariables.SCHOOL_ID, {  // HTTP POST REQUEST
            method: 'POST',
            ...
            if (response.ok) {
                ...
                if(responseJson) {
                    ...
                    if(responseJson.status === 200) {
                        ...
                        localforage.setItem("SchoolAdminToken", responseJson.token); // Store user Token

                        this.setState({ isRedirect: true });

                        // this.props.history.push('/SchoolAdmin'); // redirect to Teachers Dashboard
