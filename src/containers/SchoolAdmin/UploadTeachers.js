...
class UploadTeachers extends Component {
    ...
    handleSubmit = (event) => { // on form submit
        ...
        this.processUploadTeachers();
    };


    processUploadTeachers = (event) => { // process add teachers

        if (this.teachers.files.length < 1) { // check teachers
            this.setState({errorMessage: 'Teachers field is required'});
            // this.teachers.focus();
            return false;
        }

        var data = new FormData();
        data.append('teachers', new Blob([this.teachers.files[0]]));
        ...
        localforage.getItem('SchoolAdminToken', (err, value) => {
            ...
            if(value) {
                ...
                fetch( GeneralApis.API_SCHOOL_ADMIN_BASEURL + GeneralVariables.SCHOOL_ID +'/teachers/upload', {  // HTTP POST REQUEST
                    method: 'POST',
                    body: data,
                    headers: {
                                    ...
                                    <div className="ibox float-e-margins">
                                        ...
                                        <div className="ibox-content">
                                            ...
                                            <form ref="uploadForm" className="form-horizontal" encType="multipart/form-data" onSubmit={this.handleSubmit}>
                                                ...
                                                <div className="form-group"><label className="col-sm-2 control-label">Teachers</label>

                                                    <div className="col-sm-10">
                                                        <input
                                                            name="teachers"
                                                            ref={(input) => { this.teachers = input; }}
                                                            type="file"
